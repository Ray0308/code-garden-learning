(function defineLearningContent(root) {
  ...
  const levels = {
    ...
    18: { prerequisite:17, capabilities:['move','action','input','if','attack','sayHello'], title:'三つの影', mission:'3体の正体を判断せよ', description:'相手は正面に来るまで正体不明。input()とif/elseで3体すべてへ正しく対応しよう。', start:{x:1,y:8,direction:2}, exit:{x:1,y:1}, setup:{type:'randomMobs',positions:[{x:1,y:7},{x:1,y:5},{x:1,y:3}]}, maxSteps:16, obstacles:wallsExcept(['1,8','1,7','1,6','1,5','1,4','1,3','1,2','1,1']), starter:'', goal:'ランダムな3体を判断してゴールする' },
    19: { prerequisite:18, capabilities:['move','action','input','if','attack','sayHello'], title:'分かれ道の判断', mission:'条件式を完成させよ', description:'3体は毎回ランダム。用意された判断コードの空欄を補おう。', start:{x:1,y:8,direction:2}, exit:{x:1,y:1}, setup:{type:'randomMobs',positions:[{x:1,y:7},{x:1,y:5},{x:1,y:3}]}, maxSteps:16, obstacles:wallsExcept(['1,8','1,7','1,6','1,5','1,4','1,3','1,2','1,1']), starter:'# enemyかどうかを判断しよう', goal:'条件式を補い3体へ正しく対応する' },
    20: { prerequisite:19, capabilities:['move','action','input','if','attack','sayHello'], title:'敵なら攻撃', mission:'真の場合の処理を理解せよ', description:'ifの条件が正しいときattack()が実行される。3体の結果を観察しよう。', start:{x:1,y:8,direction:2}, exit:{x:1,y:1}, setup:{type:'randomMobs',positions:[{x:1,y:7},{x:1,y:5},{x:1,y:3}]}, maxSteps:16, obstacles:wallsExcept(['1,8','1,7','1,6','1,5','1,4','1,3','1,2','1,1']), starter:'', goal:'敵にはattack()を実行して3体を通過する' },
    21: { prerequisite:20, capabilities:['move','action','input','if','attack','sayHello'], title:'仲間なら挨拶', mission:'elseの処理を理解せよ', description:'条件に合わない仲間はelseへ進む。敵と仲間の両方へ対応しよう。', start:{x:1,y:8,direction:2}, exit:{x:1,y:1}, setup:{type:'randomMobs',positions:[{x:1,y:7},{x:1,y:5},{x:1,y:3}]}, maxSteps:16, obstacles:wallsExcept(['1,8','1,7','1,6','1,5','1,4','1,3','1,2','1,1']), starter:'', goal:'elseで仲間へsayHello()する' },
    22: { prerequisite:21, capabilities:['move','action','input','if','attack','sayHello'], title:'壊れた判断', mission:'逆になった処理を直せ', description:'敵と仲間への処理が逆になっている。失敗結果を読み、3体すべてに通用するよう直そう。', start:{x:1,y:8,direction:2}, exit:{x:1,y:1}, setup:{type:'randomMobs',positions:[{x:1,y:7},{x:1,y:5},{x:1,y:3}]}, maxSteps:16, obstacles:wallsExcept(['1,8','1,7','1,6','1,5','1,4','1,3','1,2','1,1']), starter:'# attack()とsayHello()が逆\nmob = input()\nif mob == "enemy":\n    sayHello()\nelse:\n    attack()', goal:'条件分岐の処理を修正して3体を通過する' },
    23: { prerequisite:22, capabilities:['move','action','input','if','attack','sayHello','for'], title:'三影の門', mission:'3回の判断を短く書け', description:'正体不明の3体を、forとif/elseを組み合わせた短いコードで突破しよう。', start:{x:1,y:8,direction:2}, exit:{x:1,y:1}, setup:{type:'randomMobs',positions:[{x:1,y:7},{x:1,y:5},{x:1,y:3}]}, maxSteps:16, obstacles:wallsExcept(['1,8','1,7','1,6','1,5','1,4','1,3','1,2','1,1']), starter:'', goal:'forで3回判断し、すべて正しく対応してゴールする' }
  };
  ...
  const supportByFloor = {
    ...
    18:{mode:'copy',instruction:'3体は正面まで正体不明。お手本のif/elseを3回使おう。',initialCode:'# お手本を入力して3体を判断しよう',example:'mob = input()\nif mob == "enemy":\n    attack()\nelse:\n    sayHello()\nmove()\nmove()',hints:['各相手の前でinput()します。','enemyならattack()、それ以外はsayHello()です。','同じ判断を3体分行い、最後にaction()します。']},
    19:{mode:'fill',instruction:'条件式と対応処理を補って3体を判断しよう。',hints:['条件はmob == "enemy"です。','elseの中はsayHello()です。']},
    20:{mode:'copy',instruction:'ifが真になったときのattack()に注目しよう。',initialCode:'# 3体へ同じ判断を行おう',example:'mob = input()\nif mob == "enemy":\n    attack()\nelse:\n    sayHello()\nmove()\nmove()',hints:['enemyのときだけattack()へ進みます。','3体とも正体は毎回変わります。']},
    ...
    23:{mode:'fromScratch',instruction:'forの中に入力・条件分岐・2回の移動を書こう。',initialCode:'# 3回繰り返す判断を自分で書こう',hints:['for _ in range(3):から始めます。','input()とif/elseもforの内側へインデントします。','各対応後にmove()を2回実行し、最後にaction()します。']}
  };
