(function extendPythonCommonCourse(root) {
  ...
  const specs = [
    ...
    ['記録を読み戻す','仮想読込','load("score")','保存した値を読み戻して出力せよ','load()で保存済みの値を読み込める。','save("message", "saved")\nmessage = load("message")\nprint(message)',{kind:'output',expected:'saved'}],
    ...
    ['共通編・最後の依頼','共通基礎の総合課題','変数 / 演算 / if / save','売上を計算・判定・保存せよ','単価と個数から売上を計算し、基準以上なら結果を保存して出力しよう。','price = 350\ncount = 4\ntotal = price * count\nif total >= 1000:\n    result = "success"\nelse:\n    result = "retry"\nsave("result", result)\nprint(result)',{kind:'storage',key:'result',expected:'success'}]
  ];
